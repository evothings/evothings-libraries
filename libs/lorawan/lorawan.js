...
;(function (exports) {
  ...
  function validateRegion(region) {
    ...
    // validate the things network region
    if (ttnSuffix.test(reg) && !(regions.hasOwnProperty(reg.replace(ttnSuffix, '')))) {
  ...
  Client.prototype.send = function send(devId, payload, port) {
    var topic = format('{0}/devices/{1}/down', this.appId, devId);
    var message = {
      port: port || 1
    };
    // 
  ...
  Client.prototype._connect = function _connect(connack) {
    this.ee.emit('connect', connack);
  }

  Client.prototype._error = function _error(err) {
    this.ee.emit('error', err);
  }

  Client.prototype._handleMessage = function _handleMessage(topic, message) {
  ...
  Client.prototype._eventToTopic = function _eventToTopic(name, devId, field) {
    if (devId && /[+#\/]+/.test(devId)) {
  ...
  Client.prototype._topicToEvent = function _topicToEvent(topic) {
